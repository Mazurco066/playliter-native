...
export async function chordProSongtoHtml(songs: ISong[], pdfPreview?: IPreviewData): Promise<string> {
  ...
  // PDF Preview page
  if (pdfPreview) {
    previewHtml += `<div id="ghost-preview"><span>dummy</span></div><div id="pdf-preview"><div class="svg-container"><img src="${imageAsset.localUri}" alt="PDF Preview" /></div><div class="show-info"><h3 class="show-title">${pdfPreview.title}</h3><h4 class="band-info">${pdfPreview.date} - ${pdfPreview.band}</h4><p class="show-desc">${pdfPreview.description}</p>`
